function obj = subobjective(weight, ind, idealpoint, idealpointmax, method)
...
%   A row vector of subobjective is return in both case.
    if (nargin==2)
        obj = ws(weight, ind);
    elseif (nargin==3)
        obj = te(weight, ind, idealpoint, idealpointmax);
    else
        if strcmp(method, 'ws')
            obj = ws(weight, ind);
...
%%Objectives should be regularizing when the orders of the magnitude are
%%world of difference
function obj = te(weight, ind, idealpoint, idealpointmax)
    [~, s]  = size(weight);
    indsize = size(ind,2);
    ...
    if indsize==s 
        ...
        obj     = max(bsxfun(@rdivide,weight.*part2,abs(idealpointmax-idealpoint)));
    ...
    if indsize ==1 && s > 1
        ...
        obj     = max(bsxfun(@rdivide,(weight.*part2(:,ones(1, s))),abs(idealpointmax-idealpoint)));
    ...
    if indsize > 1 && s == 1
        part2   = abs(ind-idealpoint(:,ones(1, indsize)));
        obj     = max(bsxfun(@rdivide,(weight(:,ones(1, indsize)).*part2),abs(idealpointmax-idealpoint)));
