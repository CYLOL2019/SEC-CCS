function [df, ...   % objectives
    ds, ... % parameters
    fes] = SMSEMOA(... % struct with information
...
defopts.var_crossover_prob= '0           % [0.8, 1] % variable crossover probability';
defopts.var_crossover_dist= '0           % distribution index for crossover';
defopts.var_mutation_prob = '0       % variable mutation probability';
defopts.var_mutation_dist = '0           % distribution index for mutation';
defopts.var_swap_prob     = '0          % variable swap probability';
defopts.DE_F              = '0.5% difference weight for DE';
...
[nVar rngMin rngMax isInt nObj problemCall] = initpro(initProblemStr);
df = [];
ds = [];
fes = [];
...
%size(population)
population = rand(nPop,nVar+nObj);
for popnum = 1:nPop
    population(popnum,nVar+1:nVar+nObj)=feval(problemCall,population(popnum,1:nVar));
end
% set value of outputs

ds = [ds;population(1:nPop,1:nVar)];
df = [df;population(1:nPop,nVar+1:nVar+nObj)];
...
%                 disp(sprintf('p-value regression analysis: %f', OCD_pReg));
    ...
    if mod(countEval, outputGen)==0
        ...
        if countEval >= it*1000
   
        df    = [df; population(:,nVar+1:nVar+nObj)];
        ds    = [ds; population(:,1:nVar)];
...
if useDE
    ...
    for j = 1 : nVar
        if rand(1) < variable_crossover_prob
            ...
            u1 = rand(1);
            if u1 <= 0.5
                bq = (2*u1)^(1/(variable_crossover_dist+1));
            else
                bq = (1/(2*(1 - u1)))^(1/(variable_crossover_dist+1));
            end
...
offspring(1:nVar) = max([rngMin; offspring(1:nVar)]);
           case 2
               ind = population(mypermutation(1),1:nVar).^(1+rand);
               offspring (1:nVar) = ind;
           case 3
               swap_sign = randi(4);
               ind = population(mypermutation(1),1:nVar);
               [NULL, index] = sort(ind(1:end/2),'descend');
                while 1
                  first = index(randi(K));
